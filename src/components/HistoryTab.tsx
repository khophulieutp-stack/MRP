  ...
  if (historyRecords.length === 0) {
    return (
      <div className="bg-white rounded-lg border border-slate-200 p-8 h-full flex flex-col items-center justify-center text-slate-400">
        ...
        <p className="text-sm mt-1">Chưa có lệnh phân bổ vật tư nào được tạo.</p>
      </div>
    );
  }
      ...
      <div className="p-4 lg:p-6 pb-2 shrink-0">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800">Lịch sử Lệnh Xuất Kho</h2>
              <p className="text-sm text-slate-500 mt-0.5">Danh sách các lệnh xuất kho đã được tạo.</p>
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="flex-1">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
                    Sản lượng SX: {record.khsxData?.map((k: any) => k.soLuong).reduce((a: number, b: number) => a + b, 0)}
                  </span>
                </div>
              </div>
